...
for (const scene of ir.scenes ?? []) {
  clips.push(`<div class="clip bg" data-start="${sceneOffset}" data-duration="${scene.duration}" data-track-index="0"></div>`);
  for (const layer of scene.layers ?? []) {
    const start = sceneOffset + layer.start;
    const duration = Math.max(0.001, layer.end - layer.start);
    if (layer.type === 'text') {
      clips.push(`<div class="clip title" data-start="${start}" data-duration="${duration}" data-track-index="${layer.z ?? 10}">${escapeHtml(layer.content)}</div>`);
      ...
      clips.push(`<div class="clip light-cut" data-start="${start}" data-duration="${duration}" data-track-index="${layer.z ?? 20}"></div>`);
  ...
  for (const cue of scene.subtitle_cues ?? []) {
    clips.push(`<div class="clip subtitle" data-start="${sceneOffset + cue.start}" data-duration="${Math.max(0.001,cue.end-cue.start)}" data-track-index="100">${escapeHtml(cue.text)}</div>`);
  }
  for (const track of scene.audio_tracks ?? []) {
    const src = `./assets/${path.basename(track.asset_ref)}`;
    clips.push(`<audio data-start="${sceneOffset + track.start}" data-duration="${Math.max(0.001,(track.end ?? scene.duration)-track.start)}" data-track-index="200" data-volume="${track.volume ?? 1}" src="${src}"></audio>`);
  }
...
<div data-composition-id="motion-runtime" data-width="${ir.canvas.width}" data-height="${ir.canvas.height}" data-fps="${ir.canvas.fps}" data-duration="${totalDuration}">
${clips.join('\n')}
</div></body></html>`;

fs.writeFileSync(path.join(projectDir,'index.html'), html);
const report = {provider:'hyperframes',compiler_version:'0.1.0',source_ir:irPath,total_duration:totalDuration,fps:ir.canvas.fps,warnings};
